...
#endif
{
	...
	printf("<tr><th width=100px>%s</th></tr>\n", months_ml[lang_active][this_month-1]);
	...
	for (day = 1, holidays = pm ? pm->holidays : 0;
		...
		/* special HTML code for holidays (cf. pcaldefs.h) */
		if (holidays & 01)
			printf("%s%d%s %c", HOLIDAY_PRE, day, HOLIDAY_POST,days_ml[lang_active][(day+START_BOX(this_month, this_year))%7][0]);
		else {
			/* print Sundays in bold */
			if ((day+START_BOX(this_month, this_year))%7 == 0)
				printf("<B>%d %c</B>", day,days_ml[lang_active][(day+START_BOX(this_month, this_year))%7][0]);
			/* print Saturdays in gray */
			else if ((day+START_BOX(this_month, this_year))%7 == 6)
				printf("<B><FONT color=darkgray>%d</FONT> %c</B>", day,days_ml[lang_active][(day+START_BOX(this_month, this_year))%7][0]);
			else
				printf("%d %c", day,days_ml[lang_active][(day+START_BOX(this_month, this_year))%7][0]);
			/* print associated text (count lines),
			   not for holidays */
			for (nrows = 0, pd = pm ? pm->day[day-1] : NULL;
		     	pd;
